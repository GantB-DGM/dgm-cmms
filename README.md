...
- SharePoint can't hide individual columns, so costs, internal notes, and vendor contacts live in separate lists that staff can't read. Site scoping is done by the app, so staff can technically read other sites' requests. That's a known trade-off, so a provisioning script hides every list from site navigation and search.
- Operations assigns vendors, books site visits, and records costs. The app never contacts vendors.
- PMs, SPMs, and Operations can mark requests completed, with no approval step. The requester, or anyone following the request, can press "Still a problem" for 7 days (an admin can change this). After that, it's final. Costs are optional, and a "Completed, no cost" view shows the gaps.
...
- Sites read from the IR Sites list, with one editor per field. Maintenance adds nothing to that list. It keeps its own Locations list (common areas, kitchen areas and units, with bulk-add for units). A site with a kitchen area is a kitchen site.
